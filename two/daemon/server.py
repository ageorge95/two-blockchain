...
from two.cmds.init_funcs import check_keys, two_init
from two.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from two.daemon.keychain_server import KeychainServer, keychain_commands
from two.daemon.windows_signal import kill
from two.plotters.plotters import get_available_plotters
from two.plotting.util import add_plot_directory
from two.server.server import ssl_context_for_root, ssl_context_for_server
from two.ssl.create_ssl import get_mozilla_ca_crt
from two.util.two_logging import initialize_logging
from two.util.config import load_config
from two.util.json_util import dict_to_json_str
from two.util.keychain import (
    Keychain,
...
from two.util.service_groups import validate_service
from two.util.setproctitle import setproctitle
from two.util.ws_message import WsRpcMessage, create_payload, format_response
...
service_plotter = "two plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "two": "two",
        "two_wallet": "start_wallet",
        "two_full_node": "start_full_node",
        "two_harvester": "start_harvester",
        "two_farmer": "start_farmer",
        "two_introducer": "start_introducer",
        "two_timelord": "start_timelord",
        "two_timelord_launcher": "timelord_launcher",
        "two_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["two", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("two daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered TWO_ROOT
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with TWO_ROOT: {os.environ['TWO_ROOT']}")
    ...
    if service_command == "two_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in two_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    two_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("two_daemon")
