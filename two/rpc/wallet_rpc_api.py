...
from two.pools.pool_puzzles import SINGLETON_MOD_HASH, create_p2_singleton_puzzle
...
from two.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
...
class WalletRpcApi:
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "two_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xtwo_target_address")
        pool_target = config["pool"].get("xtwo_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from two.pools.pool_wallet_info import initial_pool_state_from_dict
