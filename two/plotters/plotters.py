...
from two.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from two.plotters.chiapos import get_chiapos_install_info, plot_two
from two.plotters.madmax import get_madmax_install_info, plot_madmax
from two.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in TWO_ROOT.
    two_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within TWO_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, two_plotter, "chiapos", "Chiapos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_two(args, two_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, two_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, two_root_path, root_path)
