...
ROOT = pathlib.Path(importlib.import_module("two").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `two` file name (which is the executable)
    and the `two` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `two` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("two-blockchain"))[0]
...
# TODO: collapse all these entry points into one `two_exec` entrypoint that accepts the server as a parameter

entry_points = ["two.cmds.two"] + [f"two.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "two.util.bip39"])

if THIS_IS_WINDOWS:
    two_mod = importlib.import_module("two")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/two/util/english.txt", "two/util"))
datas.append((f"{ROOT}/two/util/initial-config.yaml", "two/util"))
datas.append((f"{ROOT}/two/wallet/puzzles/*.hex", "two/wallet/puzzles"))
datas.append((f"{ROOT}/two/ssl/*", "two/ssl"))
...
add_binary("two", f"{ROOT}/two/cmds/two.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/two/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/two/server/start_{server}.py", COLLECT_ARGS)
