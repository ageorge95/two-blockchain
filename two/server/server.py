...
from two.protocols.protocol_state_machine import message_requires_reply
from two.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from two.protocols.shared_protocol import protocol_version
from two.server.introducer_peers import IntroducerPeers
from two.server.outbound_message import Message, NodeType
from two.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from two.util.errors import Err, ProtocolError
from two.util.ints import uint16
from two.util.network import is_localhost, is_in_network
from two.util.ssl_check import verify_ssl_certs_and_keys
...
class ChiaServer:
    def __init__(
        ...
        two_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        ...
        self.two_ca_crt_path, self.two_ca_key_path = two_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.two_ca_crt_path, self.two_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.two_ca_crt_path, self.two_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSChiaConnection] = None
        try:
