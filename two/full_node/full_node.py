...
import two.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from two.consensus.block_creation import unfinished_block_to_full_block
from two.consensus.block_record import BlockRecord
from two.consensus.blockchain import Blockchain, ReceiveBlockResult
from two.consensus.blockchain_interface import BlockchainInterface
from two.consensus.constants import ConsensusConstants
from two.consensus.cost_calculator import NPCResult
from two.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from two.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from two.consensus.multiprocess_validation import PreValidationResult
from two.consensus.pot_iterations import calculate_sp_iters
from two.full_node.block_store import BlockStore
from two.full_node.bundle_tools import detect_potential_template_generator
from two.full_node.coin_store import CoinStore
from two.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
from two.full_node.hint_store import HintStore
from two.full_node.lock_queue import LockClient, LockQueue
from two.full_node.mempool_manager import MempoolManager
from two.full_node.signage_point import SignagePoint
from two.full_node.sync_store import SyncStore
from two.full_node.weight_proof import WeightProofHandler
from two.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from two.protocols.full_node_protocol import RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
from two.protocols.protocol_message_types import ProtocolMessageTypes
from two.protocols.wallet_protocol import CoinState, CoinStateUpdate
from two.server.node_discovery import FullNodePeers
from two.server.outbound_message import Message, NodeType, make_msg
from two.server.server import ChiaServer
from two.types.blockchain_format.classgroup import ClassgroupElement
from two.types.blockchain_format.pool_target import PoolTarget
from two.types.blockchain_format.sized_bytes import bytes32
from two.types.blockchain_format.sub_epoch_summary import SubEpochSummary
from two.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
from two.types.coin_record import CoinRecord
from two.types.end_of_slot_bundle import EndOfSubSlotBundle
from two.types.full_block import FullBlock
from two.types.generator_types import BlockGenerator
from two.types.header_block import HeaderBlock
from two.types.mempool_inclusion_status import MempoolInclusionStatus
from two.types.spend_bundle import SpendBundle
from two.types.transaction_queue_entry import TransactionQueueEntry
from two.types.unfinished_block import UnfinishedBlock
from two.util import cached_bls
from two.util.bech32m import encode_puzzle_hash
from two.util.check_fork_next_block import check_fork_next_block
from two.util.condition_tools import pkm_pairs
from two.util.db_synchronous import db_synchronous_on
from two.util.db_wrapper import DBWrapper
from two.util.errors import ConsensusError, Err, ValidationError
from two.util.ints import uint8, uint32, uint64, uint128
from two.util.path import mkdir, path_from_root
from two.util.profiler import profile_task
from two.util.safe_cancel_task import cancel_task_safe
...
class FullNode:
    ...
    def set_server(self, server: ChiaServer):
        ...
        if "dns_servers" in self.config:
            ...
            # dns_servers.append("dns-introducer.twochain.top")
            pass
        try:
    ...
    async def respond_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'xtwo')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
