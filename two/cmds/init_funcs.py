...
from two.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from two.util.config import (
    create_default_two_config,
    initial_config_file,
    load_config,
    save_config,
    unflatten_properties,
)
from two.util.ints import uint32
from two.util.keychain import Keychain
from two.util.path import mkdir
from two.util.ssl_check import (
...
from two.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from two.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'two keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "xtwo_target_address" not in config["farmer"]
    stop_searching_for_pool = "xtwo_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xtwo_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xtwo_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "xtwo_target_address" not in config["farmer"]:
        print(
            f"Setting the xtwo destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["xtwo_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["xtwo_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xtwo_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xtwo_target_address" not in config["pool"]:
        print(f"Setting the xtwo destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["xtwo_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["xtwo_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xtwo_target_address']} with {all_targets[0]}"
        )
    if "xtwo_target_address" not in config["timelord"]:
        print(
            f"Setting the xtwo destination for the timelord fee (2/100 plus fees)"
            f" to {all_targets[0]}"
        )
        config["timelord"]["xtwo_target_address"] = all_targets[0]
        updated_target = True
    elif config["timelord"]["xtwo_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we might not have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['timelord']['xtwo_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the XTWO destination addresses, edit the `xtwo_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    two_ca_crt, two_ca_key = get_two_ca_crt_key()
    two_ca_crt_path = ca_dir / "two_ca.crt"
    two_ca_key_path = ca_dir / "two_ca.key"
    write_ssl_cert_and_key(two_ca_crt_path, two_ca_crt, two_ca_key_path, two_ca_key)
    ...
    two_ca_crt, two_ca_key = get_two_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, two_ca_crt, two_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                two_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return two_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def two_version_number() -> Tuple[str, str, str, str]:
...
def two_minor_release_number():
    res = int(two_version_number()[2])
    print(f"Install release number: {res}")
    return res


def two_full_version_str() -> str:
    major, minor, patch, dev = two_version_number()
...
def two_init(
    ...
    create_default_two_config(root_path)
    if testnet:
        configure(root_path, "", "", "", "", "", "", "", "", testnet="true", peer_connect_timeout="")
    create_all_ssl(root_path)
    if fix_ssl_permissions:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'two keys show --show-mnemonic-seed'")
