...
from two.util.ints import uint16, uint64
from two.util.misc import format_bytes, format_minutes
from two.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total two farmed: {amounts['farmed_amount'] / units['two']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['two']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['two']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print(
                "For details on farmed rewards and fees you should run 'two start wallet' and 'two wallet show'"
            )
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'two wallet show'")
    else:
        print("Note: log into your key using 'two wallet show' to see rewards for each key")
