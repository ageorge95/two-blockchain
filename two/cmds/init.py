...
@click.option("--testnet", is_flag=True, help="Configure this two install to connect to the testnet")
...
def init_cmd(ctx: click.Context, create_certs: str, fix_ssl_permissions: bool, testnet: bool, **kwargs):
    ...
    - Shut down all two daemon processes with `two stop all -d`
    - Run `two init -c [directory]` on your remote harvester,
      where [directory] is the the copy of your Farming Machine CA directory
    - Get more details on remote harvester on Silicoin wiki:
      https://github.com/xone-network/two-blockchain/wiki/Farming-on-many-machines
    """
    from pathlib import Path
    from .init_funcs import init
    from two.cmds.passphrase_funcs import initialize_passphrase
...
if __name__ == "__main__":
    from .init_funcs import two_init
    from two.util.default_root import DEFAULT_ROOT_PATH

    two_init(DEFAULT_ROOT_PATH)
