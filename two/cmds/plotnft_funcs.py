...
from two.cmds.wallet_funcs import print_balance, wallet_coin_unit
from two.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from two.protocols.pool_protocol import POOL_PROTOCOL_VERSION
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["two"]))
        ...
        try:
            ...
            while time.time() - start < 10:
                ...
                if len(tx.sent_to) > 0:
                    ...
                    print(f"Do two wallet get_transaction -f {fingerprint} -tx 0x{tx_record.name} to get status")
                    return None
        except Exception as e:
