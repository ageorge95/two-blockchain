...
from two.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from two.types.announcement import Announcement
...
from two.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from two.wallet.puzzles.puzzle_utils import (
...
from two.wallet.secret_key_store import SecretKeyStore
from two.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_two(self, two_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if two_amount < 0:
            utxos = await self.select_coins(abs(two_amount), exclude)
        else:
        ...
        two_amount = spend_value + two_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": two_amount}]
